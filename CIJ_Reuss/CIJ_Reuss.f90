!===============================================================================
program Reuss_average
!===============================================================================
!  Compute the Reuss average of two or more elasticity tensors.
!  Usage:
!     CIJ_Reuss < [input] > [output]
!     input is: VF(i)   c11(i)   c12(i) ... c16(i)   c21(i)   ... c26(i)   ... c66(i)
!               VF(i+1) c11(i+1) c12(i) ... c16(i+1) c21(i+1) ... c26(i+1) ... c66(i+1)
!  OR
!     CIJ_Reuss [.ecs file 1] [VF 1]  [.ecs file 2] [VF 2] ... [.ecs file n] [VF n]

use anisotropy_ajn, only: CIJ_Reuss_av, CIJ_load
...
if (command_argument_count() == 0) then
   ...
   do while (ioerr == 0)
      ...
      if (ioerr > 0) then
         write(0,'(a)') 'CIJ_Reuss: Error: problem reading constants from stdin.'
         stop
      endif
      if (ioerr < 0) exit
      VF(i) = VFtemp
   ...
   call CIJ_Reuss_av(VF(1:n),C(1:n,:,:),r(1:n),Ctemp,rtemp)
