...
program iso
!===============================================================================
!  Create isotropic average for elastic constats

use anisotropy_ajn, only: CIJ_load !, CIJ_isotropic_average

implicit none

integer :: i,j,n,ifile
integer, parameter :: npoints = 224
real(8) :: C(6,6), Ciso(6,6), r
real(8) :: lon(npoints),lat(npoints)  ! points sampling a hemisphere
real(8) :: d
!character(len=80) :: file
logical :: file_exists,vel_out
...
outer:do while (i <= iargc())
      ...
      if (arg(1:1) == '-') then
         if (arg(1:2) == '-v') then      ! Only want isotropic average of velocities
            vel_out = .true.
            nopt = nopt + 1
         elseif (arg(1:2) == '--') then  ! End of options
            nopt = nopt + 1
            exit outer
   ...
   do ifile=nopt+1,iargc()
      ...
      if (.not.file_exists) then
         write(0,'(a)') 'CIJ_iso: file "' // trim(file) // '" does not exist.',&
                        'Usage: CIJ_iso < (-vel) [list of 36 ecs + density]', &
                        '   or: CIJ_iso [.ecs file]',&
                        '       -v(els) :  Only output average velocities.',&
                        '       -h(elp) :  Print help.'
         stop
